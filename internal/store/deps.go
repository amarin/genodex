...
type Store interface {
	...
	// результат. Подпись (Label) читается после списка: сущность, удалённая
	// между этими чтениями, остаётся в результате с Label = ID. Одна сущность —
	// один Hit; порядок — по таблице сущности и id (стабилен),
