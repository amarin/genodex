...
func registerRelationTools(s *server.MCPServer, relations RelationService) {
	...
	tool = mcp.NewTool(
		"relation_get",
		mcp.WithDescription("Ребро графа родства по id; результат — JSON записи. Неверный формат id или отсутствующая/приватная (для не-владельца) запись — ошибка тула; то же для ребра, ссылающегося на приватную персону (person_a/person_b); то же для ребра, ссылающегося на приватную цитату среди источников (sources[i].citation_id)"),
