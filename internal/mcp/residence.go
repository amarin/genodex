...
func registerResidenceTools(s *server.MCPServer, residences ResidenceService) {
	...
	tool = mcp.NewTool(
		"residence_get",
		mcp.WithDescription("Проживание по id; результат — JSON записи. Неверный формат id или отсутствующая/приватная (для не-владельца) запись — ошибка тула; то же для записи, ссылающейся на приватную персону (person_id); то же для записи, ссылающейся на приватную цитату среди источников (sources[i].citation_id)"),
