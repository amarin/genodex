...
- Вложенность типов административного деления проверяется сервером: тип
  дочерней единицы должен иметь ранг ниже родителя (губерния > уезд >
  волость > населённые пункты; `other` — без ограничений). Нарушение при
  создании/изменении через HTTP и MCP — ошибка валидации (`422`) с перечнем
  допустимых типов; при изменении проверяется только смена `type` или
  `parent_id`. Справочник типов — `GET /api/admin-division-types`, MCP-тул
  `division_type_list` (`internal/models/administrative_division_type_rank.go`,
  `internal/usecases/list_division_types`,
  `docs/implementation/division-type-nesting.md`).
- Веб: раздел «Справочники» со страницей «Типы административного деления»;
  шапка «Генеалогия · Данные · Справочники», каталог сущностей переименован в
  «Данные». Форма добавления единицы предлагает только допустимые типы,
  подписи «+ деление или населённый пункт» / «+ населённый пункт»,
  предупреждение о слове-типе в названии, «Создать» неактивна без названия
  и типа.
