...
[plans/2026-09-20-core-rw-roadmap.md](plans/2026-09-20-core-rw-roadmap.md). Выполнены S1–S13 (доменные правила, индексы, порт: `ctx`, `ErrNotFound`, `Delete*`, `InTx`, `Page`/`Access`, пакетная загрузка, `Search`, мелочи, контракт делений на чтение); осталось S14–S16 (запись делений, контракты записи, поиск в контрактах).
