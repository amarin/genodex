...
| `division_update` | Изменение записи: `name`/`type` обязательны и заменяются всегда; `parent_id`/`sources` — при отсутствии аргумента в вызове сохраняют текущее значение (для `parent_id` явно переданная пустая строка по-прежнему означает «корень»); результат — обновлённая единица |
...
| `relation_get` | Ребро по `id` (JSON контракта); приватная запись для не-владельца — ошибка тула; то же для ребра, ссылающегося на приватную `Person` через `person_a`/`person_b` |
...
| `residence_get` | Проживание по `id` (JSON контракта); приватная запись для не-владельца — ошибка тула; то же для записи, ссылающейся на приватную `Person` через `person_id` |
...
| `event_get` | Событие по `id` (JSON контракта); приватная запись для не-владельца — ошибка тула; то же для события, ссылающегося на приватную `Person` через `participants[i].person_id` (и в `event_search`) |
