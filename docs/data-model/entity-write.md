...
- **Приватность распространяется и на строгие ссылки на `Private`-сущность
  (обобщение правила выше).** Сущность, держащая строгую ссылку на другую
  сущность с полем `Private bool`, обязана прятать себя целиком («как
  отсутствующее», тот же принцип) не только по собственному `Private`, но
  и если референсная сущность приватна, а у вызывающего нет
  `models.AccessFull` — иначе публичная запись выдаёт факт существования и
  id приватной сущности, на которую ссылается. Конкретный экземпляр,
