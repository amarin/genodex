...
- **Каталог** (`/`, без редиректа на `/docs`) — список всех 21 сущности
  по алфавиту русского названия во множественном числе (Административное
  деление, Архивные документы, … Цитаты, Церкви), каждая строка — ссылка
  на `/<entities>` (List этой сущности). Плюс отдельная строка
  «Документация» → `/docs` (не сущность, но должна быть достижима без
  вкладок). Компонент без собственного состояния — статический массив
  `{label, path}`, сортировка `localeCompare('ru')`.
