...
- Публичный (неаутентифицированный) посетитель видит текущие вкладки
  («Населённые пункты» и т. д.) в режиме чтения. Веб-CRUD для административного
  деления уже реализован — отдельной вкладкой «Административное деление»
  (`web/src/pages/{DivisionsList,DivisionView,DivisionForm}.tsx`), не в
  `SettlementsTab`; см. `docs/plans/2026-09-23-web-divisions-crud.md`. Кнопки
...
- ~~Веб-CRUD для делений~~ — реализован отдельным проходом, отдельной
  вкладкой «Административное деление» (не в `SettlementsTab`); см.
  `docs/plans/2026-09-23-web-divisions-crud.md`.
