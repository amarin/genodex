...
сейчас (сервис пока не задеплоен за TLS-терминирующим прокси), но должно быть
...
- Одноразовость invite гарантируется только на уровне одной строки
  ...
  посторонним массово).
- Та же природа гонки — в самом bootstrap-пути: `Service.Register` читает
  `CountOwners`, и при `count == 0` пропускает проверку invite вообще
  (первый владелец создаётся без приглашения). Два конкурентных анонимных
  `POST /api/auth/register` на чистой БД оба видят `count == 0` и оба
  создают владельца (уникальность только на `login`, не на «владельцев
  ноль») — окно между чтением счётчика и `CreateOwner` дополнительно
  расширяет bcrypt-хеширование пароля (~100 мс) внутри него. Этот код —
  из этапов A/B, но до этапа C `/api/auth/*` нигде не был смонтирован на
  реальный mux, так что риск был чисто теоретическим; с этого прохода он
  реально достижим на первом запуске. Полное закрытие — той же природы,
  что у invite-гонки выше (транзакция вокруг count-check-and-create).
  Риск: только на самом первом запуске сервера, окно — доли секунды;
  deploy-блокер для публичного первого запуска, не блокер для мержа этой
  ветки — отложено на отдельный hardening-проход (не этап D, у которого
  другой фокус).
