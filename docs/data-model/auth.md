...
вызывает `CreateOwner` — это check-then-act, но `registerMu` сериализует
весь `Register` (§9), так что `ErrLoginTaken` теперь надёжен сам по себе;
constraint в БД остаётся как defense-in-depth (и как единственная защита,
если процессов, пишущих в эту БД, окажется больше одного).
...
наоборот). Хелперы остаются заготовкой для будущих MCP-тулов, которым понадобится
извлекать реквизиты запроса из контекста — текущие тулы записи
(`division_create`/`update`/`delete` в `division.go`) в них не нуждаются: весь
`/mcp` и так недостижим анонимно, как только он обёрнут `RequireAPIToken`, и
точечный гейт на уровне тула был бы дублирующей проверкой. На этапе B2
middleware и хелперы реализованы и протестированы сквозным e2e-тестом на
реальном `auth.Service`; на этапе C `RequireAPIToken` реально подключён
вокруг `/mcp` в `internal/app.New` — см. §6.
...
- `/settings` — смена пароля (успех гасит все сессии, включая текущую —
  страница отправляет на `/login`); создание invite-ссылки (копируемый URL,
  показывается один раз сразу после создания — `auth.Service` не имеет
  `ListInvites`, список ранее созданных приглашений вне этого прохода);
  список/создание/отзыв API-токенов (сырое значение — тоже один раз, сразу
  после создания).
- Публичный (неаутентифицированный) посетитель видит единую точку входа
  `/` (каталог сущностей) и страницы сущностей (`/divisions`, `/surnames`, …)
  в режиме чтения. Веб-CRUD для административного деления и фамилий уже
  реализован — каждая сущность на своём роуте
  (`web/src/pages/{DivisionsList,DivisionView,DivisionForm}.tsx`,
  `web/src/pages/{SurnamesList,SurnameView,SurnameForm}.tsx`), не во вкладках;
  см. `docs/plans/2026-09-23-web-divisions-crud.md` и
  `docs/plans/2026-09-23-entity-write-surname.md`. Кнопки
  создания/редактирования/удаления видны только вошедшему владельцу
  (`session != null` на фронте) и защищены `requireFull` на сервере — для
  Public они скрыты на уровне UI, а не только API.
...
- TLS-терминация — забота деплоя; `Secure`-флаг cookie учитывает это либо
  напрямую (`r.TLS`), либо через `-trust-proxy` за реверс-прокси — см. §4.
...
- ~~Веб-CRUD для делений~~ — реализован отдельным проходом, отдельным
  роутом «Административное деление» (не в `SettlementsTab`, который снесён
  вместе с вкладками); см. `docs/plans/2026-09-23-web-divisions-crud.md`.
...
- TOCTOU-гонка на `Service.Register` (bootstrap: два конкурентных первых
  ...
  есть — избыточен при мьютексе, но не вреден (defense-in-depth). Мьютекс
  закрывает гонку только в пределах ОДНОГО процесса: единственность
  процесса ничем не проверяется (нет flock/pidfile), и если кто-то
  запустит два процесса `genodex serve` на один `--data`, гонка вернётся.
  См. `2026-09-22-auth-hardening.md`.
