...
- Итоговое ревью: константы `NameGender` тоже получили префикс типа (`NameGenderMale/Female/Neutral`, как требует `normalization-s1s2.md` §2); `dsn` очищает путь (`filepath.Clean`), чтобы путь с `//` в начале не стал authority file-URI (тест `TestDSNCleansPath`); исправлены расхождения документации с кодом: команда запуска в `README.md` (`cmd/genodex`), четыре несуществующие связные таблицы `*_sources` в `normalization-s1s2.md` (везде `source_links`), статус `ArchiveHierarchySystem` (проект, в коде нет), описание порта в `architecture.md`, `journal_mode` в `core-read-write.md`, расширенная историческая заметка в `storage-design.md`, комментарий `models/repository.go` (ссылки на репозиторий — строгие). Не тронуто: `cmd/scraper/` в документации не описан (вне этапа).
