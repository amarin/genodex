...
- **Приёмка:** запрос без `Authorization` — `401`, не доходит до MCP-сервера;
  ...
  `401`; сбой инфраструктуры при проверке токена (например, БД недоступна) — `500`
  (не `401`, чтобы сбой был отличим от атаки); `TouchAPIToken` вызывается при успехе
...
- **Статус:** выполнено (2 задачи-коммита — `web/src/auth.ts`, `App.tsx`
  + `pages/{Login,Register,Settings}.tsx` — плюс ручная приёмка в браузере
  и синхронизация доков; см. `2026-09-22-auth-d-web.md`).
- **Файлы:** `web/src/auth.ts` (API-клиент: status/register/login/logout/
  ...
  bootstrap-редирект).
- **Приёмка:** ручной smoke в браузере (как S16), пройден на чистой БД —
  bootstrap-редирект на `/register`, регистрация первого владельца, логин,
  сессия в шапке, создание invite-ссылки, регистрация второго владельца по
  ней (без `invite` после bootstrap форма задизейблена до отправки),
  создание/отзыв API-токена (значение показывается один раз), смена пароля
  гасит сессию и отправляет на `/login`, анонимный `/settings` → `/login`,
  анонимная запись деления — `401` (доказано на уровне API этапом C;
  UI для записи делений — отдельный проход, веб-CRUD, вне этого прохода —
  см. auth.md §7).
