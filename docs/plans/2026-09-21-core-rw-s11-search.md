...
- Итоговое ревью: `storage.Normalize` теперь отбрасывает пробелы по краям (раньше `TrimSpace` был только у запроса `Search`, и термин с пробелом в начале не находился по собственному тексту; существующие индексы перестроятся при следующем `Save*`); `TrimSpace` в `Search` убран. Уточнены комментарии `maxRune` (термин с U+10FFFF сразу за префиксом вне диапазона), `searchSQL` и порта (`Label` читается после списка: удалённая между чтениями сущность остаётся с `Label = ID`); в `decisions.md` (#19) и спецификации механизм поиска описан как диапазон по `idx_search_term`. Проверка «таблицы с колонкой `private` — сущности» остаётся за `TestPrivateColumnTablesAreTheEntitiesWithFlag` (S9): условие `Search` использует `e.id`.
