...
4. **List** — дерево от корня (`fetchAdminDivisions({})` без `parent_id` ⇒
   `models.DivisionQuery.ParentID == nil` ⇒ бэк отдаёт корень,
   `internal/models/query.go:67`), дети подгружаются по клику на
   раскрывашку (`antd Tree` + `loadData`). Клик по названию узла — переход
   на View. Поиск по названию временно подменяет дерево плоским списком
   найденного (как у «Населённые пункты»).
