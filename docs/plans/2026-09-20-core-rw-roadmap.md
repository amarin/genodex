...
> Проход выполнен целиком (S1–S16). Эта карта — рабочий план (историческая запись,
> ссылки на TDD-планы этапов актуальны); итоговое описание —
> `docs/implementation/core-read-write.md`.
...
- **Приёмка:** число SQL-запросов `ListPeople`/`ListAdministrativeDivisions` не
  зависит от числа строк в пределах одного куска `IN` — до 500 значений (тест
  считает запросы через обёртку над исполнителем `Store`);
...
- **Предпосылки из S4:**
  - фикстуры `sqlstore` не пройдут `Validate()`: неканонические id (`src-1`,
    `cit-1`, `rep-1`, `p-1`), `Source` без `Reliability`, `Repository` без `Type`,
    `AdministrativeDivision.Items` со ссылкой на персону — нужен общий построитель
    канонических фикстур;
  - `ArchiveAnchor.DocumentID` (значение, пусто = нет) и `Attachment.DocumentID`
    (`*ID`, указатель на пустое = ошибка) кодируют «нет документа» по-разному —
    сценарии не должны смешивать;
  - `Validate()` возвращает первую ошибку; для списка полей в HTTP 422 понадобится
    `ValidationErrors` (решается в S15).
...
- **Предпосылка из S7:** `InUseError.Referrers` обрезан до 20 без признака
  усечения (ровно 20 неотличимо от «20 из 500»); до фиксации тела `409`
  решить, нужны ли `Total`/`Truncated`. Ошибка `Delete*`/`Get*` для
  отсутствующей сущности — голый `ErrNotFound` без вида и id (вопрос S12).
...
- **Решения этапа:**
  1. Тело `409` — `{error, referrers:[{type,id}…]}`; `Total`/`Truncated` не
     вводим: приёмке нужен только список (до 20), «20 из 500» неотличимо от
     «ровно 20» — принятое ограничение, снимать при горизонтальном срезе
     людей/событий.
  2. `GET/PUT/DELETE /api/admin-divisions/{id}`: неверный формат `id` в пути —
     `422` (не 404) через `*ValidationError` сценария; `404` — только для
     отсутствующей единицы с корректным id.
  3. PUT — полная замена `name/type/parent_id`, прочие поля модели
     сохраняются: обработчик читает текущую версию (`get_division`),
     накладывает поля запроса, пишет `update_division`; ответ 200 — слитая
     версия. Отсутствие `parent_id`/`null` → корень (сценарием `validID`).
  4. POST → `201` + созданная единица; DELETE удачный → `204` без тела;
     PUT/GET → `200`.
  ...
  6. `DivisionService` (httpapi и mcp) — единый интерфейс пяти методов; фасад
     `app.divisionService` собирает пять сценариев; `idgen.New()` подключается
     в `app`. Прокидка сценариев в `app` выполнена вместе с httpapi (иначе
     дерево не собиралось), smoke — на рубеже S15.
  7. Итог S15: деления полностью закрыты по вертикали CRUD (S13+S14+S15) —
     образец для людей/событий.
- **План:** `2026-09-22-core-rw-s15-division-contracts.md`.
...
- **Решения этапа:**
  1. Поиск — отдельный эндпоинт `GET /api/admin-divisions/search` (литеральный
     маршрут побеждает `{id}` в Go 1.22 ServeMux; закреплено тестом). Параметр
     `q`; результат — полные `[]AdminDivision`, не хиты порта `Store.Search`.
  2. Поиск префиксный, по полю `name`, куда индексируются `Name` и `Variants`
     (варианты названий участвуют). Пустой `q` после `TrimSpace` — пустой
     результат без обращения к репозиторию.
  3. Дочерние деления — на уровне репозитория (`Store.ChildrenOfDivision`);
     `list_divisions` при `ParentID != nil` берёт окна оттуда. Несуществующий
     родитель — `404` (`ErrNotFound` порта); неверный формат `parent_id` —
     `422`, репозиторий не вызывается.
  4. Хит поиска, чья единица удалена между `Search` и `GetAdministrativeDivision`
     (`ErrNotFound`), пропускается — не ошибка сценария.
  5. `DivisionService` (httpapi и mcp) — единый интерфейс шести методов; фасад
     `app.divisionService` собирает шесть сценариев.
  6. Web: поиск (`Input.Search`) и переход к дочерним («дети» / «к корню») в
     «Населённых пунктах»; `web/dist` в git не хранится (`web/.gitignore`) —
     пересобирается перед prod-запуском.
  7. Итог S16: административное деление закрыто по вертикали «чтение + запись +
     список от родителя + поиск» (S13–S16) — образец для людей/событий.
- **План:** `2026-09-22-core-rw-s16-division-search.md`.
